...
def import_history(apps, schema_editor):
    ...
    date = datetime.strptime("2015-08-25", "%Y-%m-%d")
    ...
    if Story.objects.filter(key='SRMF-99'): StoryHistory(story=Story.objects.filter(key='SRMF-99')[0],status=Status.objects.get(key='Blocked'),date=date).save()
    ...
    if Story.objects.filter(key='SRMF-59'): StoryHistory(story=Story.objects.filter(key='SRMF-59')[0],status=Status.objects.get(key='Blocked'),date=date).save()
    ...
    if Story.objects.filter(key='SRMF-29'): StoryHistory(story=Story.objects.filter(key='SRMF-29')[0],status=Status.objects.get(key='Blocked'),date=date).save()
