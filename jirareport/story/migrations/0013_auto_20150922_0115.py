...
def import_history(apps, schema_editor):
    ...
    if Story.objects.filter(key='SRMF-156'): StoryHistory(story=Story.objects.filter(key='SRMF-156')[0],status=Status.objects.get(key='Blocked'),date=date).save()
    ...
    if Story.objects.filter(key='SRMF-80'): StoryHistory(story=Story.objects.filter(key='SRMF-80')[0],status=Status.objects.get(key='Blocked'),date=date).save()
    ...
    if Story.objects.filter(key='SRMF-4'): StoryHistory(story=Story.objects.filter(key='SRMF-4')[0],status=Status.objects.get(key='R3 Validated'),date=date).save()


class Migration(migrations.Migration):

    dependencies = [
        ('story', '0012_auto_20150922_0111'),
    ]
