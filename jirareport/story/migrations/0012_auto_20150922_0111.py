...
def import_history(apps, schema_editor):
    ...
    date = datetime.strptime("2015-08-26", "%Y-%m-%d")
    ...
    if Story.objects.filter(key='SRMF-156'): StoryHistory(story=Story.objects.filter(key='SRMF-156')[0],status=Status.objects.get(key='Reopened'),date=date).save()
    ...
    if Story.objects.filter(key='SRMF-81'): StoryHistory(story=Story.objects.filter(key='SRMF-81')[0],status=Status.objects.get(key='Blocked'),date=date).save()
    ...
    if Story.objects.filter(key='SRMF-78'): StoryHistory(story=Story.objects.filter(key='SRMF-78')[0],status=Status.objects.get(key='Blocked'),date=date).save()
    ...
    if Story.objects.filter(key='SRMF-4'): StoryHistory(story=Story.objects.filter(key='SRMF-4')[0],status=Status.objects.get(key='Blocked'),date=date).save()


class Migration(migrations.Migration):

    dependencies = [
        ('story', '0011_auto_20150922_0109'),
    ]
