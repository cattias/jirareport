...
def import_history(apps, schema_editor):
    ...
    date = datetime.strptime("2015-09-08", "%Y-%m-%d")
    ...
    if Story.objects.filter(key='SRMF-95'): StoryHistory(story=Story.objects.filter(key='SRMF-95')[0],status=Status.objects.get(key='R3 Delivered'),date=date).save()
    ...
    if Story.objects.filter(key='SRMF-60'): StoryHistory(story=Story.objects.filter(key='SRMF-60')[0],status=Status.objects.get(key='R3 Delivered'),date=date).save()
    ...
    if Story.objects.filter(key='SRMF-25'): StoryHistory(story=Story.objects.filter(key='SRMF-25')[0],status=Status.objects.get(key='R3 Delivered'),date=date).save()
