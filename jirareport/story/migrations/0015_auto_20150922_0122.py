...
def import_history(apps, schema_editor):
    ...
    date = datetime.strptime("2015-09-03", "%Y-%m-%d")
    ...
    if Story.objects.filter(key='SRMF-155'): StoryHistory(story=Story.objects.filter(key='SRMF-155')[0],status=Status.objects.get(key='In Progress'),date=date).save()
