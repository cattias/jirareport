...
def import_history(apps, schema_editor):
    ...
    date = datetime.strptime("2015-08-24", "%Y-%m-%d")

    if Story.objects.filter(key='SRMF-172'): StoryHistory(story=Story.objects.filter(key='SRMF-172')[0],status=Status.objects.get(key='In Progress'),date=date).save()
