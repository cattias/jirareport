...
def import_epics(apps, schema_editor):
    ...
    e=Epic(key='SRMF-1',title='Create a new POD');e.save();e.tags.add(ACS_10)
    ...
    e=Epic(key='SRMF-10',title='Post incident investigation');e.save();e.tags.add(ACS_11, admin_services)
    ...
    e=Epic(key='SRMF-33',title='Deploy a new version of POD');e.save();e.tags.add(ACS_10)
    ...
    e=Epic(key='SRMF-36',title='Dismantle an existing Izolation Zone');e.save();e.tags.add(ACS_10)
    ...
    e=Epic(key='SRMF-55',title='Administrate applications queues (LQS/GQS)');e.save();e.tags.add(ACS_11, admin_services)
    ...
    e=Epic(key='SRMF-128',title='Apply and fallback Couchbase releases');e.save();e.tags.add(ACS_12)
    e=Epic(key='SRMF-129',title='Modify size of Couchbase environment');e.save();e.tags.add(ACS_12)
    ...
    e=Epic(key='SRMF-201',title='Improvements');e.save();e.tags.add(ACS)
    ...
    e=Epic(key='SRMF-291',title='HMP - Single S-Filer Deployment');e.save();e.tags.add(HMP_SRE)
    e=Epic(key='SRMF-293',title='HMP - Redundant S-Filer Deployment');e.save();e.tags.add(HMP_SRE)
    ...
    e=Epic(key='SRMF-428',title='Suspend traffic to a Silo');e.save();e.tags.add(ACS_11)
    e=Epic(key='SRMF-429',title='Resume the traffic to a Silo');e.save();e.tags.add(ACS_11)
